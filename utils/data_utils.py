...
# ImageNet-R uses a subset of ImageNet 1k classes
# from https://github.com/hendrycks/imagenet-r/blob/master/eval.py
...
# no mean/std values online
# scaled then cropped to square
LSUN_TRANSFORMS = {
    "train": None,
    "test": tv.transforms.Compose(
        [
...
DATASET_NAME_TRANSFORM_MAPPING = {
    ...
    "lsun": LSUN_TRANSFORMS,
    "svhn": SVHN_TRANSFORMS,
    "cifar10-c": CIFAR_10_TRANSFORMS, # just use same for corrupted as clean
    "cifar100-c": CIFAR_100_TRANSFORMS,
    "imagenet-c": IMAGENET_TRANSFORMS,
    "imagenet-r": IMAGENET_TRANSFORMS,
    "imagenet-o": IMAGENET_TRANSFORMS,
    "danbooru": IMAGENET_TRANSFORMS,
}

# corruption types for corrupted image datasets
...
class Data:
    ...
    def __init__(
        ...
        num_classes=None,
        test_only=False, 
        fast=True, # only applies to imagenet loading
        **data_kwargs
    ) -> None:
        self.name = name
        ...
        if self.name == "svhn":
            ...
            self.test_set = tv.datasets.SVHN(
                root=self.datapath,
                split="test",
                transform=self.transforms["test"],
                target_transform=self.target_transforms["test"]
        ...
        if self.name == "danbooru":
            ...
            # make the same size as imagenet-r
            indices = [i for i in range(30000)]
            self.test_set = torch.utils.data.Subset(
                self.test_set, indices
            )
        ...
        # shifted but representations of imagenet classes
        if self.name == "imagenet-r":
            
            # subset of imagenet 1k classes
            ...
            # need to replace the labels with the corresponding ImageNet 1K
            # indices, folders should just be loaded in order by PyTorch
            # use custom target_transform
            # https://pytorch.org/vision/stable/_modules/torchvision/datasets/folder.html#DatasetFolder.find_classes
            # in r_idx {0,...,199}
            def imagenet_r_swap_label(r_idx):
                assert (
                    r_idx >= 0 and r_idx <= 199
                ), "index should be in {0,...,199}"
                return IMAGENET_R_INDICES[r_idx]

            # allow manual override
            if self.target_transforms["test"] is None:
                self.target_transforms["test"] = imagenet_r_swap_label
            else:
                pass
            ...
            # not used for getitem
            self.test_set.targets = [
                imagenet_r_swap_label(s[1]) for s in self.test_set.samples
            ]


            # # 10，000 images to match imagenet val size
        ...
        # corrupted datasets
        # only produces a single pair of dataset/dataloader objects for 
        # a single severity/distortion
        # for iterating over multiple settings do so outside of the Data object
        if "-c" in self.name:
            if "cifar" in self.name:
                self.test_only = True
                print(f"{self.name} is only available for testing detection")
                assert {
                    "cifar_path", "distortion", "severity"
                }.issubset(self.data_kwargs) \
                , ("corrupted cifar requires cifar label location and other"
                    "kwargs that specify the type of corruption and degree.")

                assert self.data_kwargs["distortion"] in DISTORTIONS, \
                    "not a valid distortion"

                assert self.data_kwargs["severity"] in [1,2,3,4,5], \
                    "severity must be in range 1 to 5"

                # get labels from clean set
                if self.name == "cifar10-c":
                    temp_cifar = tv.datasets.CIFAR10
                else:
                    temp_cifar = tv.datasets.CIFAR100
                
                # use torchvision class and replace input data
                self.test_set = temp_cifar(
                    self.data_kwargs["cifar_path"],
                    train=False,
                    transform=self.transforms["test"]
                )
                ...
                inputs_path = os.path.join(
                    self.datapath,
                    self.data_kwargs["distortion"] + ".npy"
                )

                inputs = np.load(inputs_path)
                start_idx = (self.data_kwargs["severity"]-1) * 10000
                end_idx = start_idx + 10000

                # extract correct severity chunk
                inputs = inputs[start_idx:end_idx]

                # input data replaced
                self.test_set.data = inputs
        ...
        # TODO add imagenet (will be different)
            if "imagenet" in self.name:

                self.test_only = True
                print(f"{self.name} is only available for testing")
                assert {
                    "distortion", "severity"
                }.issubset(self.data_kwargs), \
                    ("corrupted imagenet requires "
                    "kwargs that specify the type of corruption and degree.")
                ...
                subfolder = self.data_kwargs["distortion"] + '/'\
                     + str(self.data_kwargs["severity"])
                self.test_set = ImageFolder(
                    root=os.path.join(self.datapath, subfolder),
                    transform=self.transforms["test"]
                )
            ...
            # add distortion and level into name
            self.name = f"{self.name}_" + self.data_kwargs["distortion"] + "_" \
                + str(self.data_kwargs["severity"])
...
if __name__ ==  "__main__":
    # danbooru = Data(
    #     "danbooru",
    #     "/work/guoxia01/danbooru2020/512px",
    #     batch_size=16,
    #     num_classes=1000,
    #     transforms=get_preprocessing_transforms(
    #         "danbooru", id_dataset_name="imagenet")
    # )

    # print(danbooru.test_set[0])
    # print(len(danbooru.test_set))

    imagnet_c = Data(
        "imagenet-c",
        "/work/guoxia01/ImageNet-unc-test/imagenet-c",
        batch_size=16,
        num_classes=1000,
        transforms=get_preprocessing_transforms(
            "imagenet-c", id_dataset_name="imagenet"),
        distortion="brightness",
        severity=3
    )
        
    print(imagnet_c.test_set[0])
    print(len(imagnet_c.test_set))
    ...
    print(f"same labels {imagenet.test_set.targets == imagnet_c.test_set.targets}")
    ...
    cifar100_c = Data(
        "cifar100-c",
        "/work/guoxia01/CIFAR_corrupted/CIFAR-100-C",
        num_classes=100,
        transforms=get_preprocessing_transforms(
            "cifar100-c", id_dataset_name="cifar100"
        ),
        cifar_path="/work/cifar",
        distortion="brightness",
        severity=3
    )

    print(
        f"same labels {cifar100.test_set.targets == cifar100_c.test_set.targets}"
    )
